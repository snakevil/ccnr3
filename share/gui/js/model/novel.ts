...
export class Novel implements INovel {
    public readonly parent: IBookshelf;
    ...
    public get loaded(): boolean | Promise<Novel> {
        return this._l;
    }
     ...
     * 章节列表。
     */
    private _c: (string | Chapter)[];

    public get chapters(): (string | Chapter)[] {
        return this._c;
    }

    public get length(): number {
        return this._c ? this._c.length : 0;
    }
    ...
    public constructor(bookshelf: IBookshelf, title: string, chapters: boolean | string[] = false) {
        ...
        if (chapters instanceof Array) {
            ...
            this._l = chapters;
            this._c = [];
            if (chapters) this.load();
        }
        this._t = 0;
    }

    public load(): Promise<Novel> {
        if (true === this._l) return Promise.resolve(this);
    ...
    public as(url: string): Novel {
        const parts = url.match(/^(.*\/)[^\/]+\/(|[\?#].*)$/);
        this.parent.as(parts[1] + parts[2]);
        return this;
    }

    public get(index: number, load: boolean = true): Chapter {
    ...
    public set(index: number, title: string, paragraphs: string[]): Chapter {
    ...
    public get last(): Chapter {
        return this._r;
    }

    public set last(chapter: Chapter) {
        const last = this._r;
        if (!last || last.index < chapter.index) this._r = chapter;
        this._t = Math.floor(+new Date() / 1000);
        this.parent.save();
    }

    public import(meta: [string, number, number, number]): Novel {
        const subs = meta[0].split("\r");
        this._a = subs[0];
        this._c[meta[1] - 1] = new Chapter(this, subs[1], meta[1]);
        if (meta[2]) this._r = new Chapter(this, subs[2], meta[2]);
        this._t = meta[3];
        return this;
    }

    public export(): [string, string, number, number, number] {
        const stats: [string, string, number, number, number] = [this.title, "", 0, 0, this._t],
            subs: [string, string, string] = [this._a, "", ""];
        if (this._c) {
            stats[2] = this._c.length;
            const last = this._c[stats[2] - 1];
            subs[1] = last instanceof Chapter ? last.title : (last as string);
        }
        if (this._r) {
