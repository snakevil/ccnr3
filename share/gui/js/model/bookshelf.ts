...
export class Bookshelf implements IBookshelf {
    ...
    public constructor(key: string) {
        ...
        try {
            ...
            this._d = db instanceof Array ? db.map(item => new Novel(this, item.shift()).import(item)) : [];
        } catch (e) {
            this._d = [];
        }
    }

    public save(): Bookshelf {
     ...
     * 获取书架单例。
     */
    public static load(): Bookshelf {
        if (!Bookshelf.$) Bookshelf.$ = new Bookshelf("       ");
        return Bookshelf.$;
    }

    public as(url: string): Bookshelf {
        this._u = url;
        return this;
    }

    public get(title: string): Novel {
    ...
    public set(title: string, author: string, chapters: string[]): Novel {
        ...
        } else this._d.push(novel.import([[author, chapters[size - 1], ""].join("\r"), size, 0, 0]));
        this.save();
        return novel;
    }

    public build(url: string, title: string, paragraphs: string[]): Chapter {
