...
document.addEventListener("readystatechange", () => {
    ...
    if (element) {
        ...
        for (let i = 0; i < element.children.length; i++) data.push(element.children[i].textContent);
        if (/\btoc\b/.test(element.className)) {
            model = bookshelf.set(element.dataset.title, element.dataset.author, data).as(url);
        } else model = bookshelf.build(url, element.dataset.title, data);
    } else model = bookshelf.as(url);

    ReactDOM.render(<CCNR3 model={model} page={+location.hash.substr(1) || 0} />, document.body.firstElementChild);
});
